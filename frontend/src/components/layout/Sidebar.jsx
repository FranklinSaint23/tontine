import { LayoutDashboard, Users, UserPlus, RefreshCcw, Banknote, Wallet, FileText, Settings, LogOut, X } from 'lucide-react';
...
const Sidebar = ({ activeTab, setActiveTab, user, onLogout, onClose }) => {
  return (
    <aside className="w-64 flex-shrink-0 flex flex-col bg-blue-600 text-white shadow-lg h-full">
      <div className="p-6 flex items-start justify-between">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-blue-700/50 rounded-lg transition-colors text-blue-200 mt-0.5"
      ...
      <div className="p-4 border-t border-blue-500/30 mt-auto">
        <div className="flex items-center gap-3 px-2">
          <div className="w-10 h-10 rounded-full bg-blue-400 flex items-center justify-center font-bold text-lg shadow-inner shrink-0">
          ...
          <button onClick={onLogout} className="p-2 hover:bg-blue-700 rounded-lg transition-colors shrink-0" title="Déconnexion">
            <LogOut size={18} />
          </button>
        </div>
      </div>
